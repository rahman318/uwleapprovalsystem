...
import SignaturePad from "../pages/SignaturePad"; // import komponen signature
...
const StaffForm = () => {
  ...
  // ✅ Pilihan 1: hantar ObjectId + nama/jawatan berasingan
...
return (
  <div className="min-h-screen flex flex-col items-center justify-start bg-gradient-to-b from-blue-50 to-blue-100 py-10 font-[Inter]">
    ...
    {/* Form Container (Besar Style Rasmi) */}
    <div className="bg-white p-8 rounded-lg shadow-md border border-gray-300 w-full max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" name="leaveStart" value={formData.leaveStart} onChange={handleChange}
                className="w-full border border-gray-400 rounded-md px-3 py-2" required />
            </div>
        ...
        {formData.requestType === "Pembelian" && (
          <div className="border border-gray-300 rounded-md p-4 bg-gray-50">
                  ...
                  <tr key={index}>
                    <td className="border border-gray-300 p-1 text-center">
                      <input type="number" value={item.qty}
                        ...
                        className="border border-gray-400 rounded w-full px-2 py-1 text-center" required />
                    </td>

                    <td className="border border-gray-300 p-1">
                      <input type="text" value={item.description}
                        onChange={(e) => handleItemChange(index, "description", e.target.value)}
                        className="border border-gray-400 rounded w-full px-2 py-1" required />
                    </td>

                    <td className="border border-gray-300 p-1">
                      <input type="text" value={item.remarks}
                        onChange={(e) => handleItemChange(index, "remarks", e.target.value)}
                        className="border border-gray-400 rounded w-full px-2 py-1" />
                    </td>
            ...
            <button type="button" onClick={addItemRow}
              className="mt-3 bg-green-700 text-white px-3 py-1 rounded hover:bg-green-800">
              + Tambah Item
            </button>
          </div>
        )}
        ...
        <div>
          ...
          <textarea name="details" value={formData.details} onChange={handleChange}
            className="w-full border border-gray-400 rounded-md px-3 py-2" rows="3" required />
        </div>
      ...
      <h4 className="font-semibold text-gray-700 mt-4">Tandatangan Staff:</h4>
<SignaturePad onSave={(sig) => setStaffSignature(sig)} />
 

        <button type="submit"
          className="w-full bg-blue-700 text-white py-2 rounded-md font-bold hover:bg-blue-800">
          Submit Request
        </button>
